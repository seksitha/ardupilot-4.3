...
void Copter::userhook_SlowLoop()
{
    ...
    if(arming.is_armed()){ 
        ...
        if(copter.rangefinder.has_orientation(ROTATION_PITCH_270) && AP::proximity()->sensor_enabled() && userCode.three_hz_timer == 10){
        
            if((copter.rangefinder_state.enabled == false && copter.rangefinder_state.alt_healthy) || (RC_Channels::get_radio_in(copter.wp_nav->ch_radar-1) < 1900 && copter.rangefinder_state.alt_healthy)){
                gcs().send_text(MAV_SEVERITY_NOTICE,"radar off");
        ...
        if(!userCode.is_home_set){
            ...
            //if(userCode.is_origin_set and !userCode.is_home_set){
                ...
                if (ret) userCode.is_home_set=true;
                gcs().send_text(MAV_SEVERITY_INFO,"origin_home_set ok: %0.2fcm ", ret ? (float)cur_ori.alt : (float)cur_ori.alt);
            //}
        }
    }

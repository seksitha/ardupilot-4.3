...
void Copter::read_rangefinder(void)
{
    ...
    for (uint8_t i=0; i < ARRAY_SIZE(rngfnd); i++) {
        ...
        if (abs(rf_state.glitch_count) >= RANGEFINDER_GLITCH_NUM_SAMPLES) {
            ...
            rf_state.glitch_count = 0;
            rf_state.alt_cm_glitch_protected = rf_state.alt_cm;
            rf_state.terrain_offset_cm = rf_state.alt_cm;
...
{
    float terrain_offset_cm = rangefinder_state.alt_cm_glitch_protected;
    rangefinder_state.terrain_offset_cm += (terrain_offset_cm - rangefinder_state.terrain_offset_cm) * (copter.g2.rangefinder_filt);

    //terrain_offset_cm = rangefinder_up_state.inertial_alt_cm + rangefinder_up_state.alt_cm_glitch_protected;
    //rangefinder_up_state.terrain_offset_cm += (terrain_offset_cm - rangefinder_up_state.terrain_offset_cm) * (copter.G_Dt / MAX(1, copter.G_Dt));

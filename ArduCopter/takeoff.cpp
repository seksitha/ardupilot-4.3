...
void Mode::auto_takeoff_run()
{
    ...
    if (!motors->armed() || !copter.ap.auto_armed) {
        ...
        auto_takeoff_no_nav_alt_cm = (inertial_nav.get_position_z_up_cm()) + 1 * 100;
        return;
    }
    ...
    if(AP_HAL::millis() - _debug_timer >= 500){
        gcs().send_text(MAV_SEVERITY_INFO,"takeoff_tar: %.2f, p_tar: %.2f" ,pos_z ,(pos_control->get_pos_target_cm()).z);
        _debug_timer = 0;
    }
    ...
    if (auto_takeoff_complete) {
        ...
        gcs().send_text(MAV_SEVERITY_INFO," stopping_alt: %.2f gps_alt:%.2f", auto_takeoff_complete_pos.z, complete_pos.z);
    }
}
...
void Mode::auto_takeoff_start(float complete_alt_cm, bool terrain_alt)
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO,"alt_takeoff: %.2f, postar: %.2f" ,auto_takeoff_complete_alt_cm , (pos_control->get_pos_target_cm()).z);
    if ((is_disarmed_or_landed() || !motors->get_interlock())) {
        // we are not flying, climb with no navigation to current alt-above-ekf-origin + wp_navalt_min
        gcs().send_text(MAV_SEVERITY_NOTICE,"takeoff No nav");

...
AP_GPS_UBLOX::_request_next_config(void)
{
    ...
    #if !defined(HAL_BUILD_AP_PERIPH)
    if(_next_message<=23 && show_all_step < 5 && _hardware_generation !=255) {
        gcs().send_text(MAV_SEVERITY_INFO,"ublox set %i ver: %i", _next_message, _hardware_generation);
        show_all_step = show_all_step + 1;
    }
    //if(show_all_step == 5) return; this will cause failing config check as it is not set all the things
    #endif

    switch (_next_message++) { // if fail it will decrement back @ below code _next_message++
    case STEP_PVT:
    ...
    case STEP_M10: {
        if (_hardware_generation == UBLOX_M10) {
            ...
            if(gps._m10_type[0]==0){ // new GPS GNSS = 41
                // BDS B1L with sbas and gnzz (M10 hollybro/ old m10 Qiotek)
                const uint8_t buf1[]={0xB5,0x62,0x06,0x8A,0x46,0x00,0x01,0x01,0x00,0x00,0x1F,0x00,0x31,0x10,0x01,0x20,0x00,0x31,0x10,0x00,0x24,0x00,0x31,0x10,0x01,0x21,0x00,0x31,0x10,0x00,0x22,0x00,0x31,0x10,0x01,0x25,0x00,0x31,0x10,0x00,0x0D,0x00,0x31,0x10,0x01,0x0F,0x00,0x31,0x10,0x00,0x12,0x00,0x31,0x10,0x01,0x14,0x00,0x31,0x10,0x01,0x05,0x00,0x31,0x10,0x00,0x21,0x00,0x11,0x20,0x08,0x01,0x00,0x21,0x30,0xC8,0x00,0x2F,0xDE};
                port->write(buf1,sizeof(buf1));
            }else{
                // BDS B1C ​​​​new version GNSS = 45
                const uint8_t buf[] ={0xB5,0x62,0x06,0x8A,0x46,0x00,0x01,0x01,0x00,0x00,0x1F,0x00,0x31,0x10,0x01,0x20,0x00,0x31,0x10,0x00,0x24,0x00,0x31,0x10,0x01,0x21,0x00,0x31,0x10,0x01,0x22,0x00,0x31,0x10,0x01,0x25,0x00,0x31,0x10,0x00,0x0D,0x00,0x31,0x10,0x00,0x0F,0x00,0x31,0x10,0x01,0x12,0x00,0x31,0x10,0x01,0x14,0x00,0x31,0x10,0x01,0x05,0x00,0x31,0x10,0x01,0x21,0x00,0x11,0x20,0x08,0x01,0x00,0x21,0x30,0xC8,0x00,0x31,0x14};
